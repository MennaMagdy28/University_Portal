...
namespace HUP.API
{
    ...
    public class CourseOfferingController : ControllerBase
    {
        ...
        public CourseOfferingController(ICourseOfferingService service, CourseOfferingMapper mapper)
        {
            _service = service;
            _mapper = mapper;
        }
        ...
        public async Task<ActionResult<CourseOfferingDto>> Create([FromBody] CreateCourseOfferingDto createDto)
        {
            ...
            var courseOffering = _mapper.ToEntity(createDto);
            await _service.AddAsync(courseOffering);
            await _service.SaveChangesAsync();

            var createdDto = await _service.GetByIdAsync(courseOffering.Id);
            return CreatedAtAction(nameof(GetById), new { id = courseOffering.Id }, createdDto);
        }
        ...
        public async Task<IActionResult> Update(Guid id, [FromBody] CreateCourseOfferingDto updateDto)
        {
            ...
            var courseOffering = _mapper.ToEntity(updateDto);
