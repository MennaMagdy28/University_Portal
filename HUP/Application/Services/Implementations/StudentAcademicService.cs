    ...
    public class StudentAcademicService : IStudentAcademicService
    {
        ...
        public async Task<IEnumerable<StudentTimetableDto>> GetStudentTimetableAsync(Guid studentId)
        {
            ...
            if (!schedules.Any())
            {
                ...
                {
                    Console.WriteLine($"Enrolled in: {enrollment.Course?.CourseCode} - {enrollment.Course?.CourseName}");
                }
            }
        ...
        public async Task<IEnumerable<StudentExamScheduleDto>> GetStudentExamScheduleAsync(Guid studentId)
        {
            ...
            return exams.Select(e => new StudentExamScheduleDto
            {
                ...
                InstructorName = GetCourseInstructor(e.Course),
        ...
        private string GetCourseInstructor(Course course)
        {
            var instructor = course.Department?.Instructors?.FirstOrDefault();
            return instructor != null ? $"د. {instructor.User?.FullName ?? "غير محدد"}" : "غير محدد";
        }
